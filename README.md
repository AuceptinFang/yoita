...
spell-lab = "2297568811"  # 事实上workshop_id才是真正的标识符
...
- `[config]` 支持 `cache_dir`、`mount_dir`
- `[steam]` 支持 `steamcmd_path`、`force_install_dir`、`app_id`、`timeout_secs`
...
- 不删除 Steam 默认目录里的缓存
- 不退订 Workshop item
- 不删除不属于 yoita 状态文件记录的 `mount_dir` 内容，即使它们不在当前配置里
